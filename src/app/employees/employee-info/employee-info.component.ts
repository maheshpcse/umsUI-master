...
export class EmployeeInfoComponent implements OnInit {
	...
	getAllEmployees() {
		this.employeeApiService.getAllEmployeesData({ userId: Number(this.userId) }).subscribe(async (response?: any) => {
	...
	openActionDialog(task?: any, empData?: any): void {
		...
		(task == 'deactivate' || task == 'restore') ? [empData['empId'], task == 'deactivate' ? 0 : 1] : [];
